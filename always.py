...
def eval(total_result):
    ...
    for i in range(3,int(total_result[1]) + 3):
        ...
        if item == 0:
            r = round(-alpha  +  (1 - alpha) * col_result[-1] , 5)
        else:
...
if __name__ == '__main__':
    ...
    #print(finger_result[648])
    
    plt.plot(finger_result[648] ,"r", markersize=1)

    plt.title('always malicious rely reputation')
    plt.xlabel("time") #xlabel、ylabel：分别设置X、Y轴的标题文字。
    plt.ylabel("score")
    
    #plt .show()
    
    plt.savefig('一直恶意.png',dpi = 120,bbox_inches='tight') 